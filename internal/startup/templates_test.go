...
func Test_CreateMeshSecurityFilesIfNotExist(t *testing.T) {
	...
	assert.Equal(t, bFiles[10].Name, common.PodCapabilitiesExist)
	assert.Equal(t, bFiles[11].Name, common.RestrictGatewayCreationPrivileges)
	assert.Equal(t, bFiles[12].Name, common.SaferAuthorizationPolicyPatternsPolicy)
