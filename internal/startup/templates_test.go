...
func Test_CreateMeshSecurityFilesIfNotExist(t *testing.T) {
	...
	assert.Equal(t, bFiles[8].Name, common.AllowMtlsPermissiveMode)
	assert.Equal(t, bFiles[9].Name, common.AvoidOverlyBroadHostsConfigurations)
	assert.Equal(t, bFiles[10].Name, common.DestinationRulePerformTLSOrigination)
	assert.Equal(t, bFiles[11].Name, common.DetectByProtocol)
	assert.Equal(t, bFiles[12].Name, common.DownstreamConnectionLimitConfigMap)
	assert.Equal(t, bFiles[13].Name, common.IngressGatewayPatchedDownstreamConnectionLimit)
	assert.Equal(t, bFiles[14].Name, common.IstioUsing3rdPartyTokens)
	assert.Equal(t, bFiles[15].Name, common.PathNormalizationInAuthorization)
	assert.Equal(t, bFiles[16].Name, common.PodCapabilitiesExist)
	assert.Equal(t, bFiles[17].Name, common.RestrictGatewayCreationPrivileges)
	assert.Equal(t, bFiles[18].Name, common.SaferAuthorizationPolicyPatternsPolicy)
