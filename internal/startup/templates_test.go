...
func Test_CreateMeshSecurityFilesIfNotExist(t *testing.T) {
	...
	assert.Equal(t, bFiles[0].Name, common.DataPlane)
	...
	assert.Equal(t, bFiles[21].Name, common.RestrictGatewayCreationPrivileges)
	assert.Equal(t, bFiles[22].Name, common.SaferAuthorizationPolicyPatternsPolicy)
