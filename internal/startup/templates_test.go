...
func Test_CreateMeshSecurityFilesIfNotExist(t *testing.T) {
	...
	assert.Equal(t, bFiles[5].Name, common.AllowMtlsPermissiveMode)
	assert.Equal(t, bFiles[6].Name, common.DestinationRulePerformTLSOrigination)
	assert.Equal(t, bFiles[7].Name, common.DetectByProtocol)
	assert.Equal(t, bFiles[8].Name, common.PodCapabilitiesExist)
	assert.Equal(t, bFiles[9].Name, common.SaferAuthorizationPolicyPatternsPolicy)
