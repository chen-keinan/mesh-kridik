...
func Test_CreateMeshSecurityFilesIfNotExist(t *testing.T) {
	...
	assert.Equal(t, bFiles[3].Name, common.AllowMtlsPermissiveMode)
	assert.Equal(t, bFiles[4].Name, common.DestinationRulePerformTLSOrigination)
	assert.Equal(t, bFiles[5].Name, common.SaferAuthorizationPolicyPatternsPolicy)
	assert.NoError(t, err)
