...
func Test_StartCli(t *testing.T) {
	...
	assert.Equal(t, files[0].Name, common.DataPlane)
	...
	assert.Equal(t, files[21].Name, common.RestrictGatewayCreationPrivileges)
	assert.Equal(t, files[22].Name, common.SaferAuthorizationPolicyPatternsPolicy)
}
