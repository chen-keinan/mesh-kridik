...
func Test_StartCli(t *testing.T) {
	...
	assert.Equal(t, files[7].Name, common.ConfigureThirdPartyServiceAccountTokens)
	assert.Equal(t, files[8].Name, common.ControlPlane)
	assert.Equal(t, files[9].Name, common.AllowMtlsPermissiveMode)
	assert.Equal(t, files[10].Name, common.AvoidOverlyBroadHostsConfigurations)
	assert.Equal(t, files[11].Name, common.ClosePort15010UnauthenticatePlaintext)
	assert.Equal(t, files[12].Name, common.ClosePort8008UnauthenticatePlaintext)
	assert.Equal(t, files[13].Name, common.DestinationRulePerformTLSOrigination)
	assert.Equal(t, files[14].Name, common.DetectByProtocol)
	assert.Equal(t, files[15].Name, common.DownstreamConnectionLimitConfigMap)
	assert.Equal(t, files[16].Name, common.IngressGatewayPatchedDownstreamConnectionLimit)
	assert.Equal(t, files[17].Name, common.IstioUsing3rdPartyTokens)
	assert.Equal(t, files[18].Name, common.PathNormalizationInAuthorization)
	assert.Equal(t, files[19].Name, common.PodCapabilitiesExist)
	assert.Equal(t, files[20].Name, common.RestrictGatewayCreationPrivileges)
	assert.Equal(t, files[21].Name, common.SaferAuthorizationPolicyPatternsPolicy)
}
